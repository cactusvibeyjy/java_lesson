			...
			IMP.request_pay({
				...
				if (rsp.success) {
					jQuery.ajax({
						...
						data: {movie_num:movie_num,cinema_num:cinema_num,seat:seat,roomNum:roomNum,person_num:person_num,running_date:running_date
							,realPri:realPri,running_time:running_time,week:week,payment:payment},
						<%-- '<%=movie_num%>,<%=cinema_num%>,<%=seat%>,<%=roomNum%>,<%=person_num%>,<%=realPri%>,<%=running_date%>,<%=running_time%>,<%=week%>,'+'KakaoPay' --%>
					}).done(function(data) {
						/* if (everythings_fine) {							
							msg = '결제가 완료되었습니다.';
